...
def plot_kline(name,symbol,kline):
    ...
    kline_plot.render(f"{name}") # 保存html

def check(context,symbol):
    info = get_stock_info(symbol)
    name = info['名称']

    # 获取近10日K线数据
    kline_end = datetime.now(timezone(timedelta(hours=8)))
    kline_start = kline_end - timedelta(days=20) + timedelta(seconds=1)
    kline = get_kline(
        symbol,
    ...
    if (
        ...
        and latest_close >= ma_close * 1.005  # 现价超过均价至少0.5%
    ):
        make_order(symbol, "market", "buy", buy_amount)
        logger.info(f"买入策略已执行")
        logger.info(f"买入{buy_amount}股{name}({symbol})")
        return
    ...
    if pos_amount > max_pos_amount + 100:  # 实际持仓量大于最大持仓量100股
        ...
        logger.info(f"卖出{buy_amount}股{name}({symbol})")
        return
...
def main(context: AccountContext):
    ...
    symbol1 = "SH.300750"
    symbol2 = "SZ.002340"
    check(AccountContext,symbol1)
    check(AccountContext,symbol2)
    # info = get_stock_info(symbol)
    # # print(info)
    # name = info['名称']
